...
public interface TrajetRepository extends ReactiveCrudRepository<Trajet, Long>, TrajetRepositoryInternal {
    ...
    @Query("SELECT * FROM trajet entity WHERE entity.application_user_id = :id")
    Flux<Trajet> findByApplicationUser(Long id);

    @Query("SELECT * FROM trajet entity WHERE entity.application_user_id IS NULL")
    Flux<Trajet> findAllWhereApplicationUserIsNull();
